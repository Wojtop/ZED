...
```
Rozmiar tabeli: ***`r nrow(inventories)`***
...
```
Tabela daje nam informację, że obecnie dostępnych jest ***`r nrow(sets)`*** zestawów Lego. Należy zwrócić uwagę, że jest to wartość mniejsza, niż liczba w wpisów w tabeli *inventories*. Wynika to z faktu, że część zetawów wystąpiła w różnych wersjach. Informacja o wersji znaduje się w tabeli *inventories*. Z tego też względu w tamtej tabeli ten sam zestaw może występować kilkukrotnie, z inną wersją.
...
```
Z powyższych danych widzimy, że obecnie w Lego mamy dostępne ***`r nrow(themes)`*** motywów, z czego aż ***`r nrow(filter(themes, is.na(parent_id)))`*** to motywy będące korzeniami swoich hierarchii.
...
```
W zbiorze tym mamy ***`r nrow(inventory_sets)`*** wpisów, opisujących ***`r nrow(complex_sets)` zestawów złożonych***.
...
```
Tabela zawiera informację o ***`r nrow(minifigs)`*** minifigurkach. Warto zwrócić uwagę, że w zbiorze występuje jedynie ***`r nlevels(minifigs$fig_name)`*** unikalnych nazw figurek, co oznacza, że część różnych figurek posiada tą samą nazwę.
...
```
Zbiór zawiera ***`r nrow(inventory_minifigs)`*** wpisów na temat figurek w ***`r nrow(minifigs_by_inventory)`*** zestawach.
...
```
Zbiór posiada ***`r nrow(inventory_parts)`*** wierszy opisujących części w zestawach.
