...
Aby to osiągnąć, zbierzmy w jeden zestawnasępujące dane: rozmiar zestawu, motyw nadrzędny, rok, liczbę kategorii klocków w zestawie, liczbę użytych kolorów w zestawie. Warto zauważyć, że nie każdy zestaw ma figurki oraz nie każdy zestaw posiada części, stąd w wynikowym zbiorze mogą wystąpić wartości puste, które musimy zastąpić zerami. 
...
```
Zbiór posida ***`r format(nrow(df),big.mark=",",scientific=FALSE)`*** wpisów, czyli tyle co liczba zetawów w tabeli *sets*.

## Korelacja pomiędzy atrybutami liczbowymi zbioru

Na poniższej wizualizacji możemy zobaczyć wykres korealcji pomiędzy poszczególnymi parametrami liczbowymi zbioru. Niestety nie można było na nim uwzględnić nazwy motywu, ze względu na jego nieliczbowy format.
...
Zbiór treningowy zawiera ***`r format(nrow(training),big.mark=",",scientific=FALSE)`*** wierszy, a testowy ***`r format(nrow(testing),big.mark=",",scientific=FALSE)`***. Warto zauważyć, że mamy w nimch nierównomiernie rozłożone klasy: klasy *FALSE* jest więcej w obydwu zbiorach.
...
Ze względu na małą moc obliczniową mojego komputera oraz długi czas trwania uczenia, zdecydowałem się nie przeprowadzać dodatkowego strojenia (podjąłem takie próby ale ich czas podważył ich zasadność). 

Nie mniej jednak, uważam że powyższy model osiągnął dobry wynik, nie dając się zwieść skrzywieniu danych. Wysokie *sensitivity* wskazuje na niewiele przypadków fałszywie pozytywnych (przyznanie figurki do zestawu, który nie powinien jej mieć). Wysokie *specificity* zapenia niewiel przypadków fałszywie negatywnych, czyli nieprzypisania figurki do zestawu. Ogólna trafność na poziomie ponad 90% oraz czułość i precyzja na poziomie ponad 90% dają stosunkowo wirygodny klasyfikator.

Powyższy klsyfikator może być używany do przewidywania, czy dany zestaw z przyszłości będzie zawierać figurki.
